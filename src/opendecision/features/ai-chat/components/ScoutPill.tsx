...
import './ScoutPill.css';

interface ScoutPillProps {
  onClick?: () => void;
  className?: string;
  highlighted?: boolean;
}

export const ScoutPill = forwardRef<HTMLButtonElement, ScoutPillProps>(
  ({ onClick, className = '', highlighted = false }, ref) => {
    return (
      <button
        ref={ref}
        className={`scout-pill ${highlighted ? 'highlighted' : ''} ${className}`}
        onClick={onClick}
        type="button"
      >
...
ScoutPill.displayName = 'ScoutPill';
