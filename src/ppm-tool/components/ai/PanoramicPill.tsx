...
import './PanoramicPill.css';

interface PanoramicPillProps {
  onClick?: () => void;
  className?: string;
  highlighted?: boolean;
}

export const PanoramicPill = forwardRef<HTMLButtonElement, PanoramicPillProps>(
  ({ onClick, className = '', highlighted = false }, ref) => {
    return (
      <button
        ref={ref}
        className={`panoramic-pill ${highlighted ? 'highlighted' : ''} ${className}`}
        onClick={onClick}
        type="button"
      >
        <span className="pill-lighting"></span>

        <span className="pill-content">
          <div className="pill-graphic-wrap">
            <svg
            ...
            <svg
              className="pill-mountains"
              viewBox="20 40 260 90"
              xmlns="http://www.w3.org/2000/svg"
            >
          ...
          <span className="pill-text">Guided</span>
        </span>
      </button>
    );
  }
...
PanoramicPill.displayName = 'PanoramicPill';

export default PanoramicPill;
